...
#  Adapted from original source at https://github.com/La0/garmin-uploader
#  Author: La0 - Bastien Abadie
...
class GarminAPI:
    ...
    def __init__(self):
        self.URL_HOSTNAME = 'https://connect.garmin.com/modern/auth/hostname'
        self.URL_LOGIN = 'https://sso.garmin.com/sso/login'
        self.URL_POST_LOGIN = 'https://connect.garmin.com/modern/'
        self.URL_PROFILE = 'https://connect.garmin.com/modern/currentuser-service/user/info'  # noqa
        self.URL_HOST_SSO = 'sso.garmin.com'
        self.URL_HOST_CONNECT = 'connect.garmin.com'
        self.URL_SSO_SIGNIN = 'https://sso.garmin.com/sso/signin'
        self.URL_UPLOAD = 'https://connect.garmin.com/modern/proxy/upload-service/upload'
        self.URL_ACTIVITY_BASE = 'https://connect.garmin.com/modern/proxy/activity-service/activity'  # noqa
        self.URL_ACTIVITY_TYPES = 'https://connect.garmin.com/modern/proxy/activity-service/activity/activityTypes' # noqa
    ...
    def authenticate(self, username, password):
        ...
        sso_hostname = None
        resp = session.get(self.URL_HOSTNAME)
        if not resp.ok:
        ...
        res = session.get(self.URL_LOGIN, params=params)
        ...
        logging.debug('Found CSRF token {}'.format(csrf_token))
        ...
        res = session.post(self.URL_LOGIN, params=params, data=data,
                           headers=headers)
        ...
        res = session.get(self.URL_POST_LOGIN, params=params, headers=headers)
        if res.status_code != 200 and not res.history:
            raise Exception('Second auth step failed.')

        # Check login
        res = session.get(self.URL_PROFILE)
        if not res.ok:
            raise Exception("Login check failed.")
        garmin_user = res.json()
        logging.debug('Logged in as {}'.format(garmin_user['username']))

        self.session = session
        return

    def upload_activity(self, gpx_file):
        """
        ...
        files = {
            "file": (gpx_file, open(gpx_file, 'rb')),
        }
        url = '{}/gpx'.format(self.URL_UPLOAD)
        res = self.session.post(url, files=files, headers=self.common_headers)

        # HTTP Status can either be OK or Conflict
        if res.status_code not in (200, 201, 409):
            if res.status_code == 412:
                logging.error('You may have to give explicit consent for uploading files to Garmin')  # noqa
            raise GarminAPIException('Failed to upload {}'.format(gpx_file))
    ...
    def set_activity_name(self, activity_id, activity_name):
        ...
        """
        url = '{}/{}'.format(self.URL_ACTIVITY_BASE, activity_id)
        data = {
    ...
    def load_activity_types(self):
        ...
        resp = requests.get(self.URL_ACTIVITY_TYPES, headers=self.common_headers)
        if not resp.ok:
        ...
        logging.debug('Fetched {} activity types'.format(len(self.activity_types)))  # noqa
        return self.activity_types
    ...
    def set_activity_type(self, activity_id, activity_type):
        ...
        type_key = types.get(activity_type)
        if type_key is None:
            logging.error("Activity type '{}' is not valid, activity type not set".format(activity_type))
            return False
